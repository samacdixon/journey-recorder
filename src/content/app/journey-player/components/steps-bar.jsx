...
class StepsBar extends React.Component {
  ...
  componentDidMount() {
    const selectedEvent = this.props.steps[this.props.index];
  ...
  handleKeydown(event) {
    if (this.props.isKeyboardEnabled) {
      if (event.keyCode === Constants.LEFT_KEY || event.keyCode === Constants.RIGHT_KEY) {
        ...
        this.props.indexClick(Math.min(this.props.selectedIndex + 1, this.props.steps.length - 1));
  ...
  componentWillReceiveProps(props) {
    const selectedEvent = props.steps[props.index];
  ...
  render() {
    ...
    const filledValue = this.props.steps[this.props.index].distance - this.props.barPaddingLeft;
